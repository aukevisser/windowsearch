...
def calc_irr_diff(datasource,response,yr_start1,yr_end1,yr_start2,yr_end2):
  ...
  if datasource == "CRU":
    ...
    #AV_08082019 changed HIDfile location to present dir for GitHub upload
    ...
    HIDfile = nc.Dataset('data/hid_v1.0.f_irr_hyde_final_ir.0.5deg.10y.1900-2005.nc','r')
...
#    lat = HIDfile.variables['lat'][:]
#    lon = HIDfile.variables['lon'][:]
  elif datasource in ["CRU_CESM","CRUv4_CESM","Berkeley_CESM","CESM","CESM_TS","MODIS_CESM_TS"]:
  ...
  #so f_irr needs to be set to 0.
    ...
    f_irr_diff = np.zeros((2,lat.shape[0],lon.shape[0]))
    f_irr_diff[0,:,:] = 0.
    f_irr_diff[1,:,:] = f_irr[-1,:,:] #Take the 2005 irrigation map for assessing 2002-2010 MODIS data
    
  elif datasource in ['CESM_TS','MODIS_CESM_TS']:
    f_irr_diff = np.zeros((2,lat.shape[0],lon.shape[0]))
    f_irr_diff[0,:,:] = 0.
    f_irr_diff[1,:,0:144] = f_irr[-1,:,144:288]
    f_irr_diff[1,:,144:288] = f_irr[-1,:,0:144]
    lon -= 180.
    
    f_irr_diff[f_irr_diff == -9999.] = np.nan
    f_irr_diff[0,:,:][np.isnan(f_irr_diff[1,:,:])] = np.nan
