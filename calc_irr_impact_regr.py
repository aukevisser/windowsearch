...
def calc_irr_impact_regr(datasource,temp_product,response,t_res,thres_irr,def_regr,yr_start1,yr_end1,yr_start2,yr_end2):
  ...
  if datasource == "HadEX2":
    ...
    bl = 7
    br = bl+1
  elif datasource in ["CESM","CRU_CESM","CRUv4_CESM","Berkeley_CESM","CESM_TS","MODIS_CESM_TS"]:
    bl = 5
    br = bl+1
  ...
  if datasource == 'CRU':
    ...
    or_file = nc.Dataset('/home/vissera/Documents/Data/orog/ETOPO1_grid_CESMres.nc','r')
    elev = or_file.variables['elev'][:] / 1000.
    orog = np.zeros((elev.shape[0],elev.shape[1]))
    orog[:,0:144] = elev[:,144:288]
    orog[:,144:288] = elev[:,0:144]
    ...
    for i in range(bl,irr_impact.shape[0]-bl):
      for j in range(br,irr_impact.shape[1]-bl):
        ...
        if( np.count_nonzero(~np.isnan(f_irr_crit[i-bl:i+br,j-bl:j+br])) >= np.ceil(0.6*(bl+br)**2) and
          ...
          df_irr_box = df_irr[i-bl:i+br,j-bl:j+br]
          dT_box = T_diff[i-bl:i+br,j-bl:j+br]
          irr_box = df_irr_box.ravel()[~np.isnan(df_irr_box.ravel()) & ~np.isnan(dT_box.ravel()) & ~np.isnan(orog[i-bl:i+br,j-bl:j+br].ravel())]
          dT_nonan = dT_box.ravel()[~np.isnan(df_irr_box.ravel()) & ~np.isnan(dT_box.ravel()) & ~np.isnan(orog[i-bl:i+br,j-bl:j+br].ravel())]
            
          # & ~np.isnan(orog[i-bl:i+br,j-bl:j+br].ravel())
          # & ~np.isnan(orog[i-bl:i+br,j-bl:j+br].ravel())
          
          arr_eff = np.zeros((irr_box.shape[0],4))
          arr_eff[:,0] = irr_box
          arr_eff[:,1] = orog[i-bl:i+br,j-bl:j+br].ravel()[~np.isnan(df_irr_box.ravel()) & ~np.isnan(dT_box.ravel()) & ~np.isnan(orog[i-bl:i+br,j-bl:j+br].ravel())]
          arr_eff[:,2] = lats[i-bl:i+br,j-bl:j+br].ravel()[~np.isnan(df_irr_box.ravel()) & ~np.isnan(dT_box.ravel()) & ~np.isnan(orog[i-bl:i+br,j-bl:j+br].ravel())]
          arr_eff[:,3] = lons[i-bl:i+br,j-bl:j+br].ravel()[~np.isnan(df_irr_box.ravel()) & ~np.isnan(dT_box.ravel()) & ~np.isnan(orog[i-bl:i+br,j-bl:j+br].ravel())]
    ...
    lats = np.tile(lat,(lon.shape[0],1)).T
    lons = np.tile(lon,(lat.shape[0],1))
    or_file = nc.Dataset('/home/vissera/Documents/Data/orog/ETOPO1_grid_5arcmin.nc','r')
    orog = or_file.variables['elev'][:]
